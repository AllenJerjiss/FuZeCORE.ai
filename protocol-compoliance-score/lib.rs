...
pub fn build_compliance_tests(json_path: &str) -> Vec<ComplianceTest> {
    let json_content = fs::read_to_string(json_path).expect("Failed to read protocol JSON file");
    let test_vec: serde_json::Value = serde_json::from_str(&json_content).expect("Invalid JSON format");
