...
# Stamps to skip re-benchmark by default (keyed by stack + env file content hash)
FORCE_RE_BENCH=0
STAMP_DIR="${STAMP_DIR:-${ROOT_DIR}/.benchmarks/stamps}"
# Toggle strictness of service-cleanup (0=tolerant (warn & continue), 1=strict (abort))
STRICT_SERVICE_CLEANUP="${STRICT_SERVICE_CLEANUP:-0}"
...
run_stack_env(){ # stack env_file
  ...
  case "$S" in
    ollama|Ollama)
      ...
      if [ $rc -ne 0 ]; then
        if [ "${STRICT_SERVICE_CLEANUP}" -eq 1 ]; then
          return $rc
        else
          warn "${S}:${envbase}:service-cleanup failed (rc=$rc) — continuing"
        fi
      fi
